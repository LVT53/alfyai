...
const mockGetCampaignAssetForServing = getCampaignAssetForServing as ReturnType<
	typeof vi.fn
>;

function makeEvent(
	user = { id: "viewer-user", role: "user" },
	assetId = "asset-1",
) {
	return {
		request: new Request(
			`http://localhost/api/campaign-assets/${assetId}/content`,
		),
		locals: { user },
		params: { id: assetId },
