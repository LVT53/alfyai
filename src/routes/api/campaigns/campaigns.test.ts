...
const mockGetEligibleCampaignForUser = getEligibleCampaignForUser as ReturnType<typeof vi.fn>;
const mockGetLatestPublishedCampaign = getLatestPublishedCampaign as ReturnType<typeof vi.fn>;
const mockRecordCampaignEvent = recordCampaignEvent as ReturnType<typeof vi.fn>;
const mockCompleteCampaignForUser = completeCampaignForUser as ReturnType<typeof vi.fn>;
...
describe('user announcement campaign routes', () => {
	...
	it('returns the single eligible campaign for the current user', async () => {
		mockGetEligibleCampaignForUser.mockResolvedValue({ id: 'campaign-1', type: 'first_run_onboarding' });

		const response = await ELIGIBLE(makeEvent());
		const body = await response.json();

		expect(response.status).toBe(200);
		expect(body).toEqual({ campaign: { id: 'campaign-1', type: 'first_run_onboarding' } });
	...
	it('returns the latest published campaign for replay without checking completion state', async () => {
		mockGetLatestPublishedCampaign.mockResolvedValue({ id: 'campaign-2', type: 'release_update' });

		const response = await LATEST(makeEvent());
		const body = await response.json();

		expect(response.status).toBe(200);
		expect(body).toEqual({ campaign: { id: 'campaign-2', type: 'release_update' } });
		expect(mockGetLatestPublishedCampaign).toHaveBeenCalledTimes(1);
	});

	it('records campaign events for the current user', async () => {
		mockRecordCampaignEvent.mockResolvedValue({ id: 'event-1', eventType: 'slide_viewed' });

		const response = await EVENT(makeEvent(
			{ eventType: 'slide_viewed', slideId: 'slide-1', metadata: { preference: 'theme' } },
			{ id: 'campaign-1' },
		));
		const body = await response.json();
	...
	it('marks a campaign completed or skipped and rejects invalid reasons', async () => {
		mockCompleteCampaignForUser.mockResolvedValue({ id: 'state-1', status: 'completed' });

		const response = await COMPLETE(makeEvent({ reason: 'completed' }, { id: 'campaign-1' }));
		const body = await response.json();

		expect(response.status).toBe(200);
		expect(body.state).toEqual({ id: 'state-1', status: 'completed' });
		expect(mockCompleteCampaignForUser).toHaveBeenCalledWith('campaign-1', 'viewer-user', 'completed');

		const invalid = await COMPLETE(makeEvent({ reason: 'later' }, { id: 'campaign-1' }));
