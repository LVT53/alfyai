...
const mockStoreCampaignSourceAsset = storeCampaignSourceAsset as ReturnType<
	typeof vi.fn
>;
...
describe("POST /api/admin/campaigns/assets/upload", () => {
	...
	it("requires admin access and stores an uploaded campaign screenshot source", async () => {
		...
		formData.set(
			"image",
			new File([Buffer.from("png-bytes")], "source.png", { type: "image/png" }),
		);
	...
	it("returns field errors when the image field is missing", async () => {
		...
		expect(body.fieldErrors).toEqual({
			image: "Campaign screenshot image is required.",
		});
