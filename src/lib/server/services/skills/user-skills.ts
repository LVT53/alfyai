...
const builtInSystemSkills = [
	{
		...
		en: {
			...
			instructions: [
				"Critique the user's plan for correctness before they execute it — do not rubber-stamp it. Use only the current message, conversation, and any linked sources; never claim to have read a document that was not provided.",
				"Find contradictions, weak assumptions, missing decisions, scope creep, and conflicts with earlier decisions.",
				"Output: a bulleted list of findings, each tagged Blocker, Major, or Minor, Blockers first; for every Blocker, name the concrete failure it causes if shipped as-is.",
				"End with one overall risk rating (Low/Medium/High) and a one-line reason.",
				"Prefer concrete fixes (reworded acceptance criteria, a renamed term, a decision to record) over vague warnings.",
				"If one fact would resolve an open question, ask it — one question, with your best-guess answer attached — instead of listing it as a gap.",
			].join("\n"),
		},
		hu: {
			displayName: "Tervkritikus",
			description:
				"Egy terv blokkoló hibáit, gyenge feltételezéseit és hiányosságait tárja fel végrehajtás előtt, súlyosság szerint rangsorolva.",
			instructions: [
				"Bíráld el a felhasználó tervét a végrehajtás előtti helyesség szempontjából — ne hagyd jóvá automatikusan. Csak a jelenlegi üzenetre, a beszélgetésre és a kijelölt forrásokra támaszkodj; soha ne állítsd, hogy olyan dokumentumot olvastál, amelyet nem kaptál meg.",
				"Keress ellentmondásokat, gyenge feltételezéseket, hiányzó döntéseket, scope creepet és a korábbi döntésekkel ütköző pontokat.",
				"Kimenet: pontokba szedett lista, minden megállapítás Blokkoló, Jelentős vagy Kisebb címkével, elöl a Blokkolókkal; minden Blokkolónál nevezd meg a konkrét hibát, amit okoz, ha változatlanul kerül bevezetésre.",
				"Zárd egy összesített kockázati besorolással (Alacsony/Közepes/Magas) egysoros indoklással.",
				"A konkrét javításokat (átfogalmazott elfogadási kritérium, átnevezett fogalom, rögzítendő döntés) részesítsd előnyben a homályos figyelmeztetésekkel szemben.",
				"Ha egyetlen tény eldöntené a nyitott kérdést, kérdezd meg — egy kérdést, a saját becsült válaszoddal együtt — ahelyett hogy hiányosságként sorolnád fel.",
			].join("\n"),
		},
		activationExamples: [
			"criticize this plan",
	...
	},
	{
		...
		en: {
			...
			description:
				"Explains an attached or linked document in plain language, preserving exact numbers, dates, and caveats — fetches a URL first if you paste one.",
			instructions: [
				"Explain the selected, attached, or linked document so the user can act on it without rereading it. If the user pastes a URL instead of attaching a document, call fetch_url on it first, then explain the fetched content.",
				"Output: one-line takeaway, then a short plain-language summary, then a detailed breakdown preserving exact dates, numbers, names, requirements, and exceptions — never flatten them into vague language.",
				"Ground every claim in the document; if the user asks something the document does not answer, say so explicitly instead of guessing.",
				"Adapt depth to the user's apparent familiarity: define terms for beginners, focus on implications for advanced users.",
				'Mark any uncertain or ambiguous claim inline as "Confidence: high/medium/low" with a one-line reason.',
				"End with a short list of decisions, risks, or follow-up questions the document implies, when useful.",
			].join("\n"),
		},
		hu: {
			...
			instructions: [
				"Úgy magyarázd el a kijelölt, csatolt vagy hivatkozott dokumentumot, hogy a felhasználó cselekedni tudjon belőle anélkül, hogy újraolvasná. Ha a felhasználó dokumentum helyett URL-t küld, előbb hívd meg a fetch_url eszközt rá, majd a lekért tartalmat magyarázd el.",
				"Kimenet: egysoros lényegi megállapítás, majd rövid közérthető összefoglaló, majd részletes bontás, amely megőrzi a pontos dátumokat, számokat, neveket, követelményeket és kivételeket — soha ne mosd össze őket homályos megfogalmazással.",
				"Minden állítást a dokumentumra alapozz; ha a felhasználó olyat kérdez, amire a dokumentum nem válaszol, mondd ki egyértelműen ahelyett, hogy találgatnál.",
				"Igazítsd a részletességet a felhasználó tudásszintjéhez: kezdőnél definiáld a fogalmakat, haladónál a következményekre fókuszálj.",
				"Minden bizonytalan vagy kétértelmű állításnál jelöld be „Megbízhatóság: magas/közepes/alacsony” címkével, egysoros indoklással.",
	...
	},
	{
		...
		en: {
			...
			description:
				"Coaches active learning with chunked explanations, recall questions, and a study plan instead of a plain summary.",
			instructions: [
		...
		hu: {
			...
			description:
				"Darabolt magyarázatokkal, visszakérdezéssel és tanulási tervvel aktív tanulást vezet le sima összefoglaló helyett.",
			instructions: [
				"A felhasználót aktív tanulásban vezesd, ne csak összefoglalj. Bontsd a témát tanulható részekre, nevezd meg az előfeltételeket, és az első részt magyarázd el, mielőtt mélyebbre mész — soha ne öntsd rá az egész témát egyszerre.",
				"Minden rész után tegyél fel egy rövid ellenőrző kérdést, majd a válasz alapján igazodj: a félreértéseket közvetlenül javítsd, és magyarázd el, miért, ne csak ismételd meg a helyes választ.",
				"Használj konkrét példákat, összehasonlításokat és kis gyakorlatokat az elvont biztatás helyett.",
				"Kimenet: minden válasz végén adj rövid „mi jön most”-ot — ismétlési javaslatot, gyakorlófeladatot vagy a következő rész nevét —, hogy a felhasználó mindig tudja a következő lépést.",
				"A válaszok legyenek elég rövidek ahhoz, hogy válaszra hívjanak, ne monológra.",
			].join("\n"),
		},
		activationExamples: [
			"help me study this",
	...
	},
	{
		...
		en: {
			...
			instructions: [
				"Help the user decide what to buy, fitted to their real constraints — not a generic best-of list. First identify budget, must-haves, dealbreakers, and compatibility or region constraints already stated.",
				"Call research_web for current prices, specs, and reviews whenever the decision depends on facts that go stale (price, availability, model-year specs) — never guess a price.",
				"Output: a comparison table across the 2-4 criteria that matter most to this user (price, key specs, standout difference), not prose paragraphs.",
				"For your top pick, always name one real weakness or who should skip it.",
				"End with a clear call — buy now / wait / skip — tied to this user's stated use case, or name the single missing fact that would decide it.",
				"Preserve concrete facts the user already gave (owned items, region, prior preferences) across the conversation.",
			].join("\n"),
		},
		hu: {
			displayName: "Vásárlási segítő",
			description:
				"Vásárlási lehetőségeket hasonlít össze a valós korlátaidhoz és aktuális árakhoz igazítva — az interneten kutat specifikációkért és értékelésekért ajánlás előtt.",
			instructions: [
				"Segíts a felhasználónak eldönteni, mit vegyen, a valós korlátaihoz igazítva — ne általános toplistát adj. Először azonosítsd a már megadott költségkeretet, kötelező elemeket, kizáró okokat és kompatibilitási vagy régiós korlátokat.",
				"Hívd meg a research_web eszközt aktuális árakért, specifikációkért és értékelésekért, amikor a döntés gyorsan elavuló tényektől függ (ár, elérhetőség, évjárat szerinti specifikáció) — soha ne találj ki árat.",
				"Kimenet: összehasonlító táblázat a felhasználó számára legfontosabb 2-4 szempont mentén (ár, kulcsspecifikációk, fő megkülönböztető jegy), ne folyószöveg.",
				"A vezető ajánlásodnál mindig nevezz meg egy valódi gyengeséget vagy azt, kinek nem ajánlott.",
				"Zárj egyértelmű döntéssel — vedd meg most / várj / hagyd ki —, a felhasználó megadott felhasználási esetéhez kötve, vagy nevezd meg az egy hiányzó tényt, amely eldöntené a kérdést.",
				"Őrizd meg a beszélgetés konkrét tényeit (tulajdonolt eszközök, régió, korábbi preferenciák) a társalgás során.",
			].join("\n"),
		},
		activationExamples: [
	...
	},
	{
		...
		en: {
			...
			description:
				"Translates or rewrites text while preserving meaning, tone, and exact terminology, names, and numbers.",
			instructions: [
		...
		hu: {
			...
			description:
				"Szöveget fordít vagy átír a jelentés, hangnem és a pontos terminológia, nevek és számok megőrzésével.",
			instructions: [
				"Fordítsd vagy írd át a felhasználó szövegét úgy, hogy megmaradjon a jelentés, szándék és célközönséghez illeszkedés. A neveket, dátumokat, számokat és formázásérzékeny részleteket tartsd változatlanul, hacsak nem kérnek mást.",
				"Fordításnál a természetes célnyelvi megfogalmazást részesítsd előnyben a szó szerinti fordítással szemben, megőrizve a regisztert és árnyalatot.",
				"Átírásnál pontosan igazodj a kért hangnemhez és médiumhoz; csökkentsd a zajt és javítsd a szerkezetet, miközben megőrzöd a felhasználó hangját.",
				"Kimenet: először a javított szöveg, egyetlen, másolásra kész blokként — utána, csak ha a változtatás lényegi vagy a felhasználó kért indoklást, egy rövid magyarázat alatta.",
				"Kétértelmű jelentés módosítása előtt vagy tegyél fel egy fókuszált kérdést, vagy add meg a legbiztonságosabb értelmezést egy egysoros megjegyzéssel a bizonytalanságról — soha ne találgass némán olyasmiben, ami megváltoztatja a jelentést.",
			].join("\n"),
		},
		activationExamples: [
			"translate this",
	...
	},
	{
		...
		en: {
			...
			description:
				"Builds an agenda, questions, and a verify/flag checklist for an upcoming appointment or call — pulls in prior context automatically.",
			instructions: [
				"Prepare the user for an appointment, meeting, call, or administrative interaction. Call memory_context to check for relevant prior context (past decisions, names, commitments) before asking the user to repeat themselves.",
				'Output, in this order: (1) the 3-5 actions that matter most before the appointment, (2) an agenda, (3) questions to ask, (4) materials to bring or send, (5) a "verify / flag" section naming anything high-stakes or rule-dependent to confirm with an official source or professional, with urgency.',
				"Never invent appointment details, eligibility rules, deadlines, or legal/medical/financial advice — flag what to verify instead.",
				"Keep phrasing usable in the moment: short, prioritized, skimmable in under a minute, since the user may read this right before walking in.",
			].join("\n"),
		},
		hu: {
			displayName: "Időpontfelkészítő",
			description:
				"Napirendet, kérdéseket és egy ellenőrzési/jelzési listát készít egy közelgő időpontra vagy hívásra — automatikusan behozza az előzményeket.",
			instructions: [
				"Készítsd fel a felhasználót időpontra, megbeszélésre, hívásra vagy ügyintézésre. Hívd meg a memory_context eszközt releváns előzményekért (korábbi döntések, nevek, vállalások), mielőtt megkérnéd a felhasználót, hogy ismételje meg magát.",
				"Kimenet, ebben a sorrendben: (1) a 3-5 legfontosabb teendő az időpont előtt, (2) napirend, (3) felteendő kérdések, (4) hozandó vagy küldendő anyagok, (5) egy „ellenőrizni / jelezni” szakasz, amely megnevezi a nagy tétű vagy szabályfüggő pontokat, amiket hivatalos forrásból vagy szakembertől kell megerősíteni, a sürgősséggel együtt.",
				"Soha ne találj ki időpontadatot, jogosultsági szabályt, határidőt vagy jogi/orvosi/pénzügyi tanácsot — inkább jelezd, mit kell ellenőrizni.",
				"A megfogalmazás legyen azonnal használható: rövid, priorizált, egy percen belül átfutható, mivel a felhasználó percekkel az időpont előtt olvashatja.",
			].join("\n"),
		},
		activationExamples: [
...
const previousBuiltInSystemSkillDefaults = {
	...
	// The on-demand skill loading rewrite (session-based skill activation
	// replaced by a per-turn catalogue line + use_skill tool / forced `$`
	// selection) shortened every seeded skill's instructions to name real
	// tools and be concrete about output shape. These entries capture the
	// exact defaults each skill carried immediately before that rewrite, so
	// seedBuiltInSystemSkillDefinitions still refreshes an unmodified
	// deployed row without touching an admin's own customization.
	"system:grill-with-docs:v4": {
		displayName: "Plan Critic",
		description:
			"Stress-tests plans against selected sources, product language, constraints, and implementation reality.",
		instructions: [
