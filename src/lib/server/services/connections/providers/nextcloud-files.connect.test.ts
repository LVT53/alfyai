...
beforeEach(() => {
	dbPath = `${tmpdir()}/alfyai-test-connections-nextcloud-${randomUUID()}.db`;
...
// assertPublicHttpsUrl (the SSRF guard) now lives in ../host-locality and is
// tested in host-locality.test.ts — its cases were folded into that suite when
// the classifier was centralized.
