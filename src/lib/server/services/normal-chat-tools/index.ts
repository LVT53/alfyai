...
const TOOL_I18N: Record<"en" | "hu", ToolI18n> = {
	en: {
		...
		map_route: {
			description:
				'Places, distances, travel times and routes on OpenStreetMap data — the ONLY tool for "how far", "how long to get there", "route/directions", "what is within 20 minutes", public transport journeys and departure times, or turning a place name into coordinates; never use image_search or research_web for those. Pass one `action`: `geocode` (query, optional near/limit), `route` (origin, destination, optional waypoints), `matrix` (origins, destinations), `isochrone` (origin, ranges_s in seconds), `transit` (public transport journey with times and lines: origin, destination, optional departure or arrive_by and max_walk_minutes) `timetable` (the next departures between two places: origin, destination, optional from, window_minutes, rows) or `journey` (mixed modes in order, e.g. bike to the station, train, walk; give `legs` plus origin/destination and either arrive_by or departure). A place is a name string or {"lat":52.52,"lng":13.4}; `mode` is drive (default), walk or bike. Example: {"action":"route","origin":"Berlin Hbf","destination":"Brandenburg Gate","mode":"walk"}. It does not know where the user is: call `location` first for their coordinates. The user already sees a map card with the FULL step-by-step directions or timeline, so summarise the journey in your answer — its shape, the key turns or changes, the times — instead of repeating every step. Narrate the structured result (summary, distance_m, duration_s, steps, legs, polygons; for transit and journey the local clock times, lines and changes) and always include "© OpenStreetMap contributors".',
