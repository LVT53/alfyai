		...
		select: () => ({
			from: () => ({
				where: () => ({
					orderBy: () => {
						...
						return {
							limit: async (count?: number) =>
								ordered.slice(0, count ?? ordered.length),
					...
					// biome-ignore lint/suspicious/noThenProperty: This sentinel preserves the mocked query-builder shape.
					then: undefined,
				}),
			}),
		}),
...
describe("semantic-embeddings service", () => {
	...
	it("upserts a semantic embedding by subject identity", async () => {
		const { upsertSemanticEmbedding, getSemanticEmbedding } = await import(
			"./semantic-embeddings"
		);
	...
	it("returns the latest embedding per subject when listing a subject set", async () => {
		const { upsertSemanticEmbedding, listSemanticEmbeddingsBySubject } =
			await import("./semantic-embeddings");
		...
		expect(Array.from(mapped.keys())).toEqual(
			expect.arrayContaining(["cluster-a", "cluster-b"]),
		);
	...
	it("deletes embeddings for specific subjects", async () => {
		const { upsertSemanticEmbedding, deleteSemanticEmbeddingsForSubjects } =
			await import("./semantic-embeddings");
	...
	it("detects when a stored embedding should be refreshed", async () => {
		const { hashEmbeddingSourceText, needsSemanticEmbeddingRefresh } =
			await import("./semantic-embeddings");
