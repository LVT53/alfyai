...
**Persona-correction note**: persona correction or deletion requests should go through the memory-profile actions in `memory.ts`/`memory-profile/`; do not create a route-local override list or revive the removed local salience repair path.
