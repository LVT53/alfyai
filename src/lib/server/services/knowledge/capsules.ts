...
function parseWorkCapsuleMetadata(
	...
	return {
		taskSummary:
			typeof metadata?.taskSummary === "string" ? metadata.taskSummary : null,
		workflowSummary:
			typeof metadata?.workflowSummary === "string"
				? metadata.workflowSummary
				: null,
		keyConclusions: Array.isArray(metadata?.keyConclusions)
			? metadata.keyConclusions.filter(
					(item): item is string => typeof item === "string",
				)
			: [],
		reusablePatterns: Array.isArray(metadata?.reusablePatterns)
			? metadata.reusablePatterns.filter(
					(item): item is string => typeof item === "string",
				)
			: [],
		sourceArtifactCount:
			typeof metadata?.sourceArtifactCount === "number" &&
			Number.isFinite(metadata.sourceArtifactCount)
				? Math.max(0, Math.trunc(metadata.sourceArtifactCount))
				: legacySourceArtifactIds.length,
		outputArtifactCount:
			typeof metadata?.outputArtifactCount === "number" &&
			Number.isFinite(metadata.outputArtifactCount)
				? Math.max(0, Math.trunc(metadata.outputArtifactCount))
				: legacyOutputArtifactIds.length,
	};
}

export function mapWorkCapsuleFromArtifactRow(
	row: WorkCapsuleArtifactRow,
): WorkCapsule {
	const metadata = parseWorkCapsuleMetadata(
...
export async function upsertWorkCapsule(params: {
	...
	const meaningfulConversationTitle = isPlaceholderConversationTitle(
		conversation.title,
	)
		? null
	...
	const workflowSummary = [
		sourceArtifactIds.length > 0
			? `Used ${sourceArtifactIds.length} source document(s).`
			: null,
		outputArtifacts.length > 0
			? `Produced ${outputArtifacts.length} saved result(s).`
			: null,
	]
		.filter(Boolean)
		.join(" ");
	const keyConclusions = [
		meaningfulConversationTitle
			? `Project theme: ${meaningfulConversationTitle}`
			: null,
		sourceArtifactIds.length > 0
			? "The workflow depends on attached source documents."
			: null,
		outputArtifacts.length > 0
			? "The conversation produced reusable written outputs."
			: null,
	].filter((item): item is string => Boolean(item));
	const reusablePatterns = [
		sourceArtifactIds.length > 0
			? "Start from the existing source set and update with fresh material."
			: null,
		outputArtifacts.length > 0
			? "Reuse prior output structure as a template before rewriting details."
			: null,
	].filter((item): item is string => Boolean(item));

	const metadata = {
		taskSummary:
			taskSummary || meaningfulConversationTitle || conversationBaseName,
		workflowSummary:
			workflowSummary || "Conversation generated reusable knowledge.",
		keyConclusions,
		reusablePatterns,
	...
	const contentText = [
		...
		keyConclusions.length > 0
			? `Key conclusions: ${keyConclusions.join(" ")}`
			: null,
		reusablePatterns.length > 0
			? `Reusable patterns: ${reusablePatterns.join(" ")}`
			: null,
	]
		.filter(Boolean)
		.join("\n");
