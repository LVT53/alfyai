	...
	it("wraps the admin campaign authoring contract", async () => {
		const fetchImpl = vi
			...
			.mockResolvedValueOnce(
				new Response(
					JSON.stringify({
						campaigns: [{ id: "campaign-1", name: "Welcome" }],
					}),
					{
						status: 200,
			...
			.mockResolvedValueOnce(
				new Response(
					JSON.stringify({ campaign: { id: "campaign-2", type: "first_run" } }),
					{
						status: 201,
						headers: { "Content-Type": "application/json" },
					},
			...
			.mockResolvedValueOnce(
				new Response(
					JSON.stringify({ campaign: { id: "campaign-1", slides: [] } }),
					{
						status: 200,
						headers: { "Content-Type": "application/json" },
					},
			...
			.mockResolvedValueOnce(
				new Response(
					JSON.stringify({ campaign: { id: "campaign-1", name: "Updated" } }),
					{
						status: 200,
						headers: { "Content-Type": "application/json" },
					},
			...
			.mockResolvedValueOnce(
				new Response(
					JSON.stringify({
						campaign: { id: "campaign-1", status: "published" },
					}),
					{
						status: 200,
						headers: { "Content-Type": "application/json" },
			...
			.mockResolvedValueOnce(
				new Response(
					JSON.stringify({
						campaign: { id: "campaign-1", status: "archived" },
					}),
					{
						status: 200,
						headers: { "Content-Type": "application/json" },
			...
			.mockResolvedValueOnce(
				new Response(
					JSON.stringify({ campaign: { id: "first-run" }, created: true }),
					{
						status: 200,
						headers: { "Content-Type": "application/json" },
					},
		...
		await expect(fetchAdminCampaigns(fetchImpl)).resolves.toEqual([
			{ id: "campaign-1", name: "Welcome" },
		]);
		await expect(
			createAdminCampaign(
				{ type: "first_run", releaseVersion: "1.0.0" },
				fetchImpl,
			),
		).resolves.toEqual({
			id: "campaign-2",
		...
		await expect(fetchAdminCampaign("campaign-1", fetchImpl)).resolves.toEqual({
			id: "campaign-1",
			slides: [],
		});
		await expect(
			updateAdminCampaign("campaign-1", { name: "Updated" }, fetchImpl),
		).resolves.toEqual({
			id: "campaign-1",
			name: "Updated",
		});
		await expect(
			publishAdminCampaign("campaign-1", fetchImpl),
		).resolves.toEqual({
			id: "campaign-1",
			status: "published",
		});
		await expect(
			archiveAdminCampaign("campaign-1", fetchImpl),
		).resolves.toEqual({
			id: "campaign-1",
			status: "archived",
		});
		await expect(
			deleteAdminCampaignDraft("campaign-1", fetchImpl),
		).resolves.toBeUndefined();
		await expect(
			duplicateAdminCampaign("campaign-1", fetchImpl),
		).resolves.toEqual({ id: "campaign-copy" });
	...
	it("wraps the user campaign delivery contract", async () => {
		const fetchImpl = vi
			...
			.mockResolvedValueOnce(
				new Response(
					JSON.stringify({
						campaign: { id: "eligible", type: "first_run_onboarding" },
					}),
					{
						status: 200,
						headers: { "Content-Type": "application/json" },
			...
			.mockResolvedValueOnce(
				new Response(
					JSON.stringify({
						event: { id: "event-1", eventType: "slide_viewed" },
					}),
					{
						status: 201,
						headers: { "Content-Type": "application/json" },
			...
			.mockResolvedValueOnce(
				new Response(
					JSON.stringify({ state: { id: "state-1", status: "completed" } }),
					{
						status: 200,
						headers: { "Content-Type": "application/json" },
					},
		...
		await expect(
			completeCampaign("eligible", "completed", fetchImpl),
		).resolves.toEqual({
			id: "state-1",
			status: "completed",
		});
