...
// in the composer and the knowledge list. Two writers feed it:
//
//  1. the SSR shell payload (`app-shell.ts` -> `(app)/+layout.svelte`), because
...
//     every real upload.
//
// Until either arrives the registry default applies, which is the same number
// `MAX_FILE_UPLOAD_SIZE` defaults to in `src/lib/server/env.ts`.
