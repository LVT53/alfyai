...
describe('CampaignCropModal', () => {
	...
	it('localizes the cropper chrome and mobile crop metadata', () => {
		...
		expect(screen.getByRole('dialog', { name: 'Kampány képernyőkép kivágása' })).toBeInTheDocument();
		expect(screen.getByText('9:16 mobil kivágás')).toBeInTheDocument();
		expect(screen.getByRole('slider', { name: 'Nagyítás' })).toBeInTheDocument();
		expect(screen.getByRole('button', { name: 'Visszaállítás' })).toBeInTheDocument();
