...
if (!process.env.SESSION_SECRET)
	process.env.SESSION_SECRET = "placeholder-secret-32-chars-long!!";
...
import {
	getOrCreateSession,
	mirrorMessage,
} from "../src/lib/server/services/honcho";
...
async function main() {
	...
	const identityNamespace =
		process.env.HONCHO_IDENTITY_NAMESPACE || "database-path-derived";
	...
	if (!apiKey) {
		console.log(
			"[BACKFILL] HONCHO_API_KEY is not set; using local no-auth client mode.",
		);
	}
	if (process.env.HONCHO_ENABLED !== "true") {
		throw new Error(
			"HONCHO_ENABLED=true is required so the central Honcho adapter mirrors messages.",
		);
	}
	...
	console.log(
		"[BACKFILL] Honcho will process these asynchronously for reasoning.",
	);
	process.exit(0);
}
