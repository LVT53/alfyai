...
if (!process.env.SESSION_SECRET)
	process.env.SESSION_SECRET = "seed-placeholder-session-secret-12345678";
...
async function main() {
	...
	if (!email || !password) {
		console.error(
			"Usage: npx tsx scripts/seed-admin.ts --email=<email> --password=<password> [--name=<name>] [--admin]",
		);
		process.exit(1);
	}
	...
	if (existing.length > 0) {
		...
		if (makeAdmin) {
			...
			console.log(
				`User ${email} already exists with role: ${existing[0].role}`,
			);
		}
		process.exit(0);
	}
