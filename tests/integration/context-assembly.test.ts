...
} from "../../src/lib/server/services/knowledge/store/core";
import { createMessage } from "../../src/lib/server/services/messages";
import { estimateTokenCount } from "../../src/lib/utils/tokens";

const _SMALL_FILE_CONTENT = "This is a small file content. ".repeat(100);
const _LARGE_FILE_CONTENT = "This is line {n} of a large document. ".repeat(
	500,
);
...
describe("Context Assembly Integration Tests", () => {
	...
	beforeEach(async () => {
		const conversation = await createConversation(
			testUserId,
			"Test Context Assembly Conversation",
		);
	...
	it("uses a valid context compression snapshot instead of covered raw session turns", async () => {
		...
		await createContextCompressionSnapshot({
			...
			snapshot: {
				...
				currentState:
					"The old exchange was compressed into a validated snapshot.",
				importantDecisions: [
					"Use the compact snapshot for covered older turns.",
				],
	...
	afterAll(async () => {
		await db
			...
			.where(eq(artifactChunks.userId, testUserId));
	...
	describe("Small File Context Assembly", () => {
		it("should include full content for files under small file threshold (<5K chars)", async () => {
			...
			expect(result.contextStatus.promptArtifactCount).toBeGreaterThanOrEqual(
				1,
			);
		});
	...
	describe("Large File Context Assembly", () => {
		it("should handle large files with chunked content (>10K chars)", async () => {
			...
			expect(result.inputValue).toContain(
				"Line content for large file testing",
			);
		});
	...
	describe("Token Budget Enforcement", () => {
		it("should respect document token budget for attachments", async () => {
			...
			const attachmentMatch = result.inputValue.match(
				/## Current Attachments[\s\S]*?(?=## |$)/,
			);
			if (attachmentMatch) {
		...
		it("should apply compaction when context exceeds target", async () => {
			...
			for (let i = 0; i < 10; i++) {
				const artifact = await createArtifact({
					...
					mimeType: "text/plain",
					contentText: `Large content for compaction test file ${i}. `.repeat(
						200,
					),
					sizeBytes: 5000,
				});
	...
	describe("Context Assembly Edge Cases", () => {
		...
		it("should handle files with special characters", async () => {
			const specialContent =
				'Special chars: àáâãäåæçèéêë ñ 中文 🎉 <script>alert("xss")</script>';
