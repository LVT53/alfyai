...
- In the `research_web` tool result diagnostics, expect `selectedSourceCount > 0` and `evidenceCandidateCount > 0`.
...
| `ATLAS_V2_MAX_WORDS_OVERVIEW` / `_IN_DEPTH` / `_EXHAUSTIVE` | No | `1100` / `2800` / `5500` | Hard word ceiling for a v2 report per profile, executive summary and Limitations included | Raise only when a longer report is genuinely wanted | The writer is given a matching per-section sentence cap, and a deterministic post-cap drops trailing sentences — uncited ones first — if the model overshoots. Env only |
...
| `ATLAS_V2_WRITER_CONCURRENCY` | No | `3` | v2 sections written in parallel | Lower it when the local model is contended | Clamped to 1-8. Env only |
...
| `ROUTING_GTFS_FEEDS` | No | empty (= the shipped catalogue) | Which GTFS feeds each region's ORS `public-transport` profile is built from, serving `map_route`'s `transit` / `timetable` actions. Empty or `catalogue` uses [`gtfs-catalogue.ts`](src/lib/server/services/routing/gtfs-catalogue.ts): **national** coverage for `hungary` (21 feeds — MÁV/GYSEV rail, Volánbusz coaches, BKK Budapest, city operators), `ireland-and-northern-ireland` (the NTA's 104-agency feed) and `netherlands` (OVapi, incl. NS rail). To take control: `regionId=url|url,regionId=url` — `\|` separates the feeds of one region, `,` separates regions, and `regionId=catalogue` opts one region back in | Leave it **empty** in production | A region you name explicitly gets exactly those feeds; a region you do not name gets none. ORS takes one `gtfs_file` string, but GraphHopper 4.14 comma-splits it and loads each path as its own feed, so the profile is built from the comma-joined list of the feeds that downloaded. Adding feeds to the region behind `ORS_BASE_URL` promotes it to a managed container (see below) |
| `ROUTING_GTFS_FEED_EXCLUDE` | No | empty | `regionId:feedId` pairs dropped from the resolved set (`regionId:*` drops a whole region) | Set `hungary:mav-gysev` unless you have filed MÁV's GTFS request form | **Licence note:** the `mav-gysev` feed is MÁV's own data relayed by `menetbrand.com`, and MÁV asks GTFS users to file its (free) request form. Excluding it drops rail from Hungarian journeys; Volánbusz coaches and the city networks stay |
| `ROUTING_GTFS_REFRESH_DAYS` | No | `7` | Default staleness before a feed is re-downloaded and its public-transport graph rebuilt | Leave at the default | A catalogue feed's own cadence wins (BKK daily, small city feeds fortnightly). The rebuild is only ever *started* between 03:00 and 05:00 local, because it restarts the region's container; an admin "Refresh timetable" ignores that window. Refreshes send `If-None-Match` / `If-Modified-Since`, so an unchanged feed costs a 304 |
| `ROUTING_GTFS_MAX_MB` | No | `600` | Size cap for a single GTFS download | Raise it only for a genuinely larger feed | Feeds publish no checksum, so integrity rests on an exact `Content-Length` match *when the server sends one* (several Hungarian city feeds are generated per request and send none) plus the zip magic number. A feed that fails costs that operator's trips only — the region still builds from the rest |
