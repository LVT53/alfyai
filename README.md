...
| `ROUTING_GTFS_FEEDS` | No | empty | `regionId=url` pairs (comma-separated) giving each region ONE GTFS feed, which becomes an ORS `public-transport` profile serving `map_route`'s `transit` / `timetable` actions. Example: `hungary=https://go.bkk.hu/api/static/v1/public-gtfs/budapest_gtfs.zip,ireland-and-northern-ireland=https://www.transportforireland.ie/transitData/Data/GTFS_All.zip,netherlands=http://gtfs.ovapi.nl/nl/gtfs-nl.zip` | Set it for the regions whose timetables you want | ORS takes exactly one feed per profile, so a country with several operators needs a pre-merged feed. The Hungarian URL above is **Budapest only** — MÁV and Volánbusz publish no open national feed, so intercity rail and coaches are missing. Adding a feed to the region behind `ORS_BASE_URL` promotes it to a managed container (see below) |
| `ROUTING_GTFS_REFRESH_DAYS` | No | `7` | How old a downloaded feed may get before its public-transport graph is rebuilt | Match the publisher's cadence (the Dutch feed refreshes every few days) | The rebuild is only ever *started* between 03:00 and 05:00 local, because it restarts the region's container. An admin "Refresh timetable" ignores that window |
| `ROUTING_GTFS_MAX_MB` | No | `600` | Size cap for a single GTFS download | Raise it only for a genuinely larger feed | Feeds publish no checksum, so an exact `Content-Length` match is the only integrity check |
