...
The dual gate that decides whether memory is active for a turn (`memory-controls.ts`). The **master toggle** (`users.memoryEnabled`) turns all memory off for a user; **incognito** (`conversations.memoryIncognito`) excludes a single conversation. `isMemoryActiveForConversation` is the single source of truth, enforced on the read side at every read call site (an inactive turn injects no baseline memory section and is not offered the `memory_context` recall tool on the main chat path) and the write side (judge, consolidation, re-curation), and fails open so a controls outage never wipes recall. Incognito is **one-way**: it is armed once, from the mask button on the landing page, before the conversation exists (the flag rides in on the creation request), and stays on for that conversation's whole life. `PATCH /api/conversations/{id}` refuses `memoryIncognito: false` with a 409 whatever the conversation's state, and accepts `true` only while the conversation has no messages; there is no switch in the composer, the "+" menu or the face's popover. To be remembered again the user starts a new chat.
_Avoid_: read-only gate, write-only gate, per-turn silent memory when off, incognito that still writes, a control that offers to turn incognito off

**Incognito Artifact Containment**:
The other half of the incognito promise: the FILES an incognito conversation produced or was given never reach another conversation. It is enforced once, in `getArtifactOwnershipScope` (`knowledge/store/core.ts`), which every user-scoped artifact query derives its answer from — so an incognito conversation's artifacts are absent from the Knowledge library, from workspace search, from the semantic candidate pool evidence selection draws on, from another conversation's working set, from `read_generated_file`, and from the document families and version counts a later chat is told about. Three deliberate exceptions, each named at its call site: the conversation being served is in its own scope (inside an incognito chat everything works); id-scoped authorization (`getArtifactForUser`) sees everything, because the ids only ever come from a scoped query; and administration — deletion, export, erasure — sees everything, or an incognito conversation's files would become undeletable. Deleting the conversation takes its artifacts with it (`deleteConversationWithCleanup`). The flag lives on the chat and the current setting governs — but it is one-way (see **Memory Master Toggle & Memory Incognito**), so in practice "the current setting" is the one the conversation was created with: there is no path that exposes an incognito conversation's earlier files. `tests/cross-cutting/incognito-artifact-containment.test.ts` holds both the behaviour and a guard over new queries.
_Avoid_: per-caller incognito filters, an incognito file in the library, a family or version count that counts an incognito version, a scope that hides rows from the delete path

**Incognito Conversation Containment**:
The same boundary for the CHAT itself: an incognito conversation's messages, title, summary and anything derived from them never reach another conversation. It is enforced once, in `buildConversationContextScopeCondition` (`services/conversation-scope.ts`), the conversation-side twin of `getArtifactOwnershipScope` — a `where` term every cross-conversation CONTEXT read adds, so `memory_context` in `history` mode (candidate search, and the detail read that opens one by id), the project-folder reference context, the folder lookup a query resolves to, the sibling promotion that reads a whole recent dialogue into a prompt, and the sibling summaries all skip incognito chats. The exception is the conversation being served, which stays in its own scope: inside an incognito chat its own page, its own summary and its own folder still work. The write side is gated separately and was already: the judge refuses to learn from an incognito turn (`memory-judge/dispatch.ts`, `index.ts`), and `memory_events` — the behavioral log a later turn's ranking counts — is dropped for an incognito conversation in `recordMemoryBehaviorEvent`. What incognito does NOT hide is the user's own UI: the sidebar, workspace search and the home screen list the chat for the person who had it (the sidebar badges it), because incognito hides a chat from the assistant, not from its owner. The per-conversation summary is still written and read INSIDE the chat, like its files: "saved-but-untracked" means contained, not unrecorded. Deleting the conversation takes its turns and its summary with it. `tests/cross-cutting/incognito-conversation-containment.test.ts` holds the behaviour and a guard over new queries, with the UI listings and the admin sweeps named on its allow-list.
_Avoid_: per-caller incognito filters, an incognito chat in a history search, a folder sibling list that counts an incognito chat, a scope that hides a conversation from its own page or from deletion
...
A short status-like sentence or phrase emitted while a response is still being prepared or reasoned through. It should be visually separated from neighboring interim steps while live, removed from the completed answer surface, and remain inspectable later through **Thought** when it was part of the persisted **Thinking Trace**. Per [ADR-0056](adr/0056-interim-thought-steps-are-durable-turn-state.md) a step is durable **Normal Chat Turn Completion** state, not transient UI decoration or a re-rendering of the trace: it is produced deterministically, from a real tool/context event, or by a bounded control-model reading of a reasoning chunk, and it is append-only and traversable in conversation history. A classified step's visible text is a short **constrained, entity-grounded summary** (per the ADR-0056 2026-08-16 amendment): the control model may paraphrase the reasoning for readability but must keep at least one verbatim content-word of the anchored span and introduce no claim absent from it — with the closed activity class kept as a secondary icon/tag, and a plain phase label as the fallback when no verbatim tether exists. Every step carries a **Thought Step Anchor**; a step that cannot name one is not emitted, a class implying an external action may originate only from a real event (never from reasoning-text), and a summary must be **entailed by** its anchored span — enforced by the ADR-0056 faithfulness audit, whose production-enable gate is ≥95% faithful with zero contradictions and zero fabricated action claims.
...
- Tool usage guidance — when to call a tool, its argument shape, an example, output handling, and failure behaviour — lives on each tool's own description (`TOOL_I18N` in `src/lib/server/services/normal-chat-tools/index.ts`, English and Hungarian), not in a message-content-selected block of the system prompt. Tool AVAILABILITY, not the current turn's wording or language, governs whether the model sees that guidance; there is no "Normal Chat Guidance Pack" selector to keep in sync across languages. `buildOutboundSystemPrompt` retains only genuinely turn-scoped guidance — response language, reasoning depth, the injected system time context, and the connections framing — see [ADR-0055](docs/adr/0055-tool-usage-guidance-lives-in-the-tool-interface.md).
...
**Pipeline note (Phase B of the v3-only consolidation, ADR 0062 amendment, ADR 0063):** Atlas now runs one content pipeline, v3, exclusively. The original pipeline ("v1" below) and its ADR 0062 rebuild ("v2") were deleted; their modules, stage lists, and marker vocabulary are historical — kept only so an already-published v1 or v2 report keeps rendering, and described here for that reason. Terms below marked **(historical, v1)** describe a deleted module or stage list; do not use them as a guide for new work. v3's own vocabulary (evidence bank, answer table, critic) lives in [ADR 0063](adr/0063-atlas-v3-reasons-from-an-evidence-bank-not-from-search-excerpts.md); the terms later phases made central — **Atlas Local Source** (Phase C), **Atlas Evidence Bank** and **Atlas Seed Recheck** (Phase D, parent-bank reuse) — are defined below.
...
**Atlas Turn**:
A **Normal Chat Turn** that produces an **Atlas** through an enforced multi-stage research pipeline. Unlike a regular turn where the model decides its own tool flow, an Atlas Turn runs a fixed server-controlled sequence. **(historical, v1)** V1 ran decompose, search, curate, synthesize, integrate, assemble, audit, then deterministic rendering; that pipeline was deleted in Phase B of the v3-only consolidation. Atlas now runs v3's own fixed sequence instead (ask, research, outline, answer, write, critic, verify, render — [ADR 0063](adr/0063-atlas-v3-reasons-from-an-evidence-bank-not-from-search-excerpts.md)), but the model still fills in content within server-owned stages rather than owning orchestration. Its kickoff assistant message flows through **Normal Chat Turn Completion** with memory enrichment explicitly skipped; the background completion records usage, source counts, file-production links, and generated-file ids on the Atlas job. It is not a parallel background subsystem and does not create a second completion turn.
_Avoid_: deep research job, research workflow, background research task, research mode

**Atlas Profile**:
The depth and duration setting for an **Atlas Turn**. Three profiles exist: **Overview** (Áttekintés), **In-Depth** (Részletes), and **Exhaustive** (Kimerítő). **(historical, v1)** In v1 a profile controlled search-query breadth, accepted web-source cap, stage output budget, and prompt posture; v3 uses the same three profiles to size its own knobs instead (researcher concurrency, pages per question, critic rounds — see `docs/atlas.md`). A profile changes budgets and posture, not the Atlas architecture, and quality gates are non-negotiable regardless of profile — even Overview cannot ship unsupported certainty.
_Avoid_: research depth, research mode, depth level, research tier

**Atlas Quality Gate**:
A non-negotiable check within the **Atlas Turn** pipeline that prevents unsupported certainty from shipping silently. **(historical, v1 mechanics)** V1's version requested one audit-driven revision pass before final rendering, and downgraded model-generated critical audit markers to warnings so the pipeline still rendered with an addendum; only structural markers (such as zero accepted sources) failed the job outright. v3's equivalent is its verify stage plus deliberate abstention (ADR 0063): if the goal test cannot be met, the report says so rather than shipping unsupported certainty. Either way, the Atlas ships with prominent Limitations rather than silent overconfidence. This replaces the old Deep Research plan-approval gate: instead of checking a prediction upfront, it checks the actual output.
_Avoid_: plan approval, research checkpoint, pass checkpoint, coverage gate, hard fail on model-generated critical markers

**Atlas Honesty Marker** (historical, v1 — module deleted in Phase B of the v3-only consolidation):
A v1 visible severity-coded indicator in an **Atlas** showing where the audit found source gaps, contradictions, language drift, or other verification concerns. Honesty Markers were derived from the **Atlas Basis** verification step, not from model self-assessment. v3 has no Honesty Marker or Basis Marker surface; it emits paragraph text, inline `[[cite:n:c|s|i]]` tokens, tables, and source chips instead (ADR 0063). A stored v1 report's Honesty Markers still render — the rendering path is kept — but nothing produces new ones.
_Avoid_: confidence badge, citation verdict, audit label, quality score

**Atlas Basis**:
The provenance contract for factual output in an **Atlas**: the assembled report is audited against the accepted source pool, search limitations, and local-source set before rendering. **(historical, v1 mechanics)** V1 stored compact audit markers and source metadata rather than a separate per-field evidence database; v3's equivalent provenance contract is the evidence bank plus per-claim citation tokens (ADR 0063). It is part of the output contract, not decorative metadata.
_Avoid_: citation list, evidence appendix, source ledger, audit trail

**Atlas Evidence Pack** (historical, v1 — module deleted in Phase B of the v3-only consolidation):
A compact, structured evidence unit created from accepted **Atlas Web Sources** and **Atlas Local Sources** before final synthesis. An Evidence Pack grouped what a source or evidence cluster supported, the relevant excerpt or summary, source authority, conflicts or limitations, and the report facet it helped cover. It was the model-facing memory for later Atlas stages and rounds. v3's equivalent model-facing memory is its evidence bank of structured claims and quotes (ADR 0063), not an Evidence Pack.
_Avoid_: evidence database, raw source pile, search dump, vector memory, citation pile

**Atlas Writer Evidence Card** (historical, v1 — module deleted in Phase B of the v3-only consolidation):
A compact, final-writing evidence unit distilled from **Atlas Evidence Packs** for the report-writing step. It carried the source identity, authority, relevant facts, limitations, conflicts, and likely supported report sections in a form the model could synthesize from without rereading raw fetched pages. v3's writer reads directly from the evidence bank instead (ADR 0063).
...
The durable checkpoint unit for an **Atlas Turn**. **(historical, v1 mechanics)** V1 wrote one completed round checkpoint after audit and before deterministic rendering, containing the curated source pool, compressed findings, usage, quality diagnostics, and document-source summary. v3 checkpoints per phase instead (research rounds land at `round_number = 10 + r`, the render checkpoint carries the curated source pool and citations — ADR 0063), keeping the same durable-checkpoint intent rather than adding stage-local persistence or treating each round as a separate Atlas Turn.
_Avoid_: research pass, workflow pass, iteration, cycle

**Atlas Evidence Bank**:
The one thing that crosses from research into reasoning in an **Atlas Turn** (v3, ADR 0063): the sources read, the verbatim quotes each read produced (with ids the writer cites), and the structured claims those quotes state, each claim `verified` (two independent publishers), `single`, `contested` or `open`. Page text never leaves the bank; later stages see quotes and claims only. Every source records when it was read (`retrievedAt`). The bank is checkpointed per research round and, from Phase D, snapshotted on the verify checkpoint as the capped bank the report was written from — which is what an **Atlas Continue** or **Atlas Revise** child reuses, after an **Atlas Seed Recheck**. An **Atlas Fork** never reuses it: quotes are extracted for a goal, and the parent's goal is not the fork's.
_Avoid_: evidence pack (v1), source pool, research scratchpad

**Atlas Seed Recheck** (the **Freshness Window**):
The step that decides which of a parent's **Atlas Evidence Bank** sources a Continue or Revise child may cite as they are. A web source is time-sensitive when it carries a claim with no period and no date, or one whose latest year is this year or last, or quotes with no claims from an aggregator or weak tier; a source whose claims all name an explicit older period (a 2019 statute, a 2023 statistic) is not. A source is **fresh** — trusted without a re-read — when it is not time-sensitive or was read inside the action's window: 14 days for Continue (`ATLAS_V3_SEED_FRESHNESS_DAYS`), 0 for Revise, so a Revise rechecks every time-sensitive source. Everything else is re-read live (bypassing the tool-result cache and Parallel's own cache) within one research round's page budget, the parent's cited sources first: **confirmed** (every quote still stated, kept), **changed** (unstated quotes dropped, the fresh page read for the question again), or **unreachable** (dropped; its figures become questions for round one). What the budget cannot reach is **dropped, never trusted**. The user's documents are re-resolved under the child's scope instead, and re-read if they changed since. The outcome is reported as `qualityDiagnostics.seed`.
_Avoid_: cache, stale check, revalidation

**Atlas Continue**:
A lifecycle action that extends an existing **Atlas** with more research. It creates a new version in the same document family. The child reuses the parent's **Atlas Evidence Bank** after an **Atlas Seed Recheck** with a 14-day window, rewrites the parent's memo (claims filtered to what survived; the parent's prose answer is not carried), sees the parent's outline, and is told the parent's title, headings and verdict — the verdict for orientation only, never as evidence. The new **Atlas Turn** builds on what was already found rather than starting from scratch.
_Avoid_: research further, extend research, resume research

**Atlas Fork**:
A lifecycle action that starts a new **Atlas** with a modified or related query from the same conversation. It creates a new document family and re-researches: it reuses NO part of the parent's **Atlas Evidence Bank**, memo or outline (quotes extracted for the parent's goal would bend the new direction back toward it), and sees only the parent's title and verdict, for orientation. It does inherit the user's documents the parent read, re-read for the fork's own questions. The original Atlas remains unchanged.
_Avoid_: branch research, alternative research, split research

**Atlas Revise**:
A lifecycle action that re-runs an **Atlas** to catch what changed, producing a new version in the same document family. The old version remains in document history. The child reuses the parent's **Atlas Evidence Bank** but with a freshness window of 0 — every time-sensitive source is rechecked live before it may be cited — sees the parent's outline for structure, does NOT reuse the parent's memo (its answer would anchor the rewrite), and still researches: its questions re-verify the parent's key figures and look for anything newer than its date. A parent from before v3 (a v1 or v2 report) seeds every lifecycle action the same way: its report's web sources are read again as seed pages; none of its text is trusted unread.
...
**Atlas Resume**:
The behavior where a failed or retried **Atlas Turn** preserves its durable checkpoint and job state rather than losing the completed audited source/report summary. **(historical, v1 mechanics)** V1 retry reran the bounded pipeline from the job and parent/checkpoint context; v3's `readAtlasV3ResumeState` ignores a foreign checkpoint schema and restarts fresh instead of reusing a v1/v2 checkpoint it cannot read (ADR 0063). A lifecycle child writes its seeded bank as research round 0, so a retried child resumes it and never runs the **Atlas Seed Recheck** twice. Explicit cancellation discards partial state and does not resume, on either pipeline.
...
**Atlas Local Source**:
A **Library Document** the user chose for an **Atlas Turn**: an explicit attachment on the kickoff user message, a **Linked Context Source** snapshotted onto that message, or — on a Continue, Revise or Fork — one the parent job read or was given (an inherited source, which degrades to a Limitations line rather than failing the job when it no longer resolves). Automatic working-set documents are NOT Atlas Local Sources — a citable evidence bank contains only material the user chose. v3 resolves them at worker time through the knowledge boundary under the job conversation's strict incognito scope, reads each once (its best passages for the ask's questions, at most 12 documents), and files what it quotes into the evidence bank as `user_document` evidence verified like any quote. All of them share one publisher, `user-documents`, so the user's documents are one independent voice: with one published source a figure is corroborated, alone it is single-source. They are never stale-listed, and they appear under a "Your Library" subsection of the Sources section as library chips numbered in the same `[n]` sequence as the web sources. An explicit one that cannot be read fails the job. **(historical, v1 mechanics)** V1 (`atlas/sources.ts`, deleted) also included readable active working-set documents with lower authority and read summaries rather than document text.
_Avoid_: knowledge attachment, uploaded research input, library artifact citation

**Atlas Web Source**:
A web page discovered through **Parallel Search** during the **Atlas Turn**'s research work. Web sources are the primary source pool for an Atlas and appear in the Atlas under a "Web Sources" subsection of the Sources section. Text discovery and page fetch both run through Parallel. Atlas availability gates on `PARALLEL_API_KEY` (`missing_parallel`). **(historical, v1/v2)** V1 and v2 also ran a separate Brave-backed image-search path for report images; v3 has no image web-source path.
_Avoid_: search result, fetched page, web citation, SearXNG result

**Atlas Source Acquisition** (historical, v1 — module deleted in Phase B of the v3-only consolidation):
The `atlas/search.ts` module that turned research queries into accepted, enriched, deduplicated **Atlas Web Sources**. It ran search → converge → enrich: it queried **Parallel Search**, converged results within a round (adult-content and unusable-snippet filtering plus canonical URL dedup), and enriched accepted sources (title hygiene, page-excerpt fetch) before they entered the source pool, deduping on a canonical URL key (`atlas/source-url.ts`, also deleted). v3's researcher stage reads through `research_web`/`fetch_url` directly (ADR 0063) instead of a separate acquisition front end.
...
- An **Atlas** draws on both **Atlas Web Sources** (Parallel) and **Atlas Local Sources** (the kickoff message's explicit linked sources and composer attachments, and on Continue/Revise the parent's own local sources — never automatic working-set documents). Memory is background context, not a citable source.
...
- The web-search backend is the **Parallel Search API** (Turbo tier), reached through the `research_web` tool (input `{query}`). Reading a specific page (a pasted URL, in-page specs, a PDF/JS page) is the separate always-on `fetch_url` tool, which uses **Parallel Extract** (cached by default) — it is not an automatic escalation after search. SearXNG was removed in [ADR-0052](docs/adr/0052-replace-searxng-web-research-with-parallel-search.md); do not describe SearXNG or a self-hosted search container as current. The `research_web` model/citation contract (grounded web candidates/metadata, `sourceType:"web"`) is unchanged — only the backend swapped.
- Parallel Turbo/Extract calls appear in analytics as synthetic `usage_events` model rows `parallel:turbo` / `parallel:extract` (flat $1/1k). These are intentional cost-attribution rows, not real provider models.
