...
**Pipeline note (Phase B of the v3-only consolidation, ADR 0062 amendment, ADR 0063):** Atlas now runs one content pipeline, v3, exclusively. The original pipeline ("v1" below) and its ADR 0062 rebuild ("v2") were deleted; their modules, stage lists, and marker vocabulary are historical — kept only so an already-published v1 or v2 report keeps rendering, and described here for that reason. Terms below marked **(historical, v1)** describe a deleted module or stage list; do not use them as a guide for new work. v3's own vocabulary (evidence bank, answer table, critic) lives in [ADR 0063](adr/0063-atlas-v3-reasons-from-an-evidence-bank-not-from-search-excerpts.md) and will get full CONTEXT.md terms as later phases (local sources, parent-bank reuse) land.
