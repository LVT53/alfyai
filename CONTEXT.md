...
**Atlas Local Source**:
A **Library Document** from the user's Knowledge Library that the **Atlas Turn** includes in its source pool alongside web sources. They appear in the Atlas under a "Your Library" subsection of the Sources section. **(historical, v1 mechanics)** V1 (`atlas/sources.ts`, deleted) snapshotted explicitly selected **Linked Context Sources** and composer attachments to the kickoff user message with the highest authority boost, and additionally included readable active working-set documents and parent Atlas seed sources automatically with lower authority. v3's local-sources work (a later phase of the v3-only consolidation) narrows this deliberately: only the kickoff message's explicit attachments, its snapshotted linked sources, and (on Continue/Revise) the parent's own local sources are included — automatic working-set inclusion is intentionally dropped, so a citable evidence bank contains only material the user chose.
