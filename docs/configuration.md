...
| `E2E_RESET_DATABASE` | unset (behaves as `false`) | When set to the literal string `true` **and** `E2E_DATABASE_PATH` points somewhere other than the default `<repo>/data/playwright-e2e-chat.db`, `globalSetup` deletes that database file (plus its `-wal`/`-shm` siblings) before running migrations, instead of the in-place `DELETE FROM` wipe it always does otherwise. Ignored on the default path, because deleting the file there would strand the already-started `webServer` on an unlinked inode (see the docstring on `wipeUserData` in `tests/e2e/global-setup.ts`) | Whoever runs `npx playwright test` against a non-default `E2E_DATABASE_PATH` and wants a schema-level reset rather than a data wipe |
