...
| `DOCUMENT_EXTRACTION_STALE_ATTEMPT_MS` | No | `900000` | Heartbeat silence after which an attempt is reclaimed (60000–3600000) | Raise it if extractions legitimately run longer | Effective value is `max(this, MINERU_JOB_TIMEOUT_MS × 2)`, so raising the MinerU timeout raises this too |
