...
5. **Parent Atlas seeds** - Continue, Revise, and Fork may use parent compressed findings and curated sources as seed context, but gap-fill decisions must not treat parent evidence as fresh by default. Stale parent evidence can guide searches; it cannot silently satisfy a current factual claim.
   > **Amended by ADR 0063 Phase D (2026-09-24): what "fresh" means.** Parent evidence is **fresh** — usable without a re-read — when its source is not time-sensitive (every claim it carries names an explicit older period, or it is a primary/press page quoted without claims) or when it was read inside the action's window: 14 days for Continue, 0 for Revise (so a Revise re-reads every time-sensitive source). Anything else is re-read live, bypassing both the tool-result cache and Parallel's own cache; a quote the live page no longer states is dropped, and a time-sensitive source the recheck budget cannot reach is dropped rather than trusted. Evidence used without a re-read is disclosed in the job's `qualityDiagnostics.seed` (the `trusted` count), and volatile figures stay dated inline as for any other source. A Fork reuses no parent evidence at all.
