...
## Amendment: server stream-timeline marks persist to `messageAnalytics` (M1)

`messageAnalytics` gains three nullable integer columns — `firstByteMs`, `firstThinkingMs`, `firstTokenMs` — populated from the existing **server** `SERVER_STREAM_TIMELINE_MARKS` (`src/lib/services/stream-timeline.ts`): `firstByteMs` from `FIRST_UPSTREAM_EVENT` (falling back to `MODEL_STREAM_REQUEST` when the upstream never produced an event), `firstThinkingMs` from `FIRST_THINKING`, and `firstTokenMs` from `FIRST_VISIBLE_TOKEN`. All three are milliseconds elapsed since turn start **as measured on the server**; there is no browser→server timing channel, so these are not browser-network-inclusive and must not be presented as such.

This is Context Preparation Timing's sibling for the model-generation phase of a turn: content-free duration diagnostics, not turn control flow. The same invariant from this ADR's body applies unchanged — a missing, malformed, or throwing mark read must never fail, delay, retry, or otherwise alter a turn. `stream-completion.ts` extracts marks through a guarded reader that degrades to `undefined` on any anomaly, and the `messageAnalytics` write is the existing best-effort, `.catch`-guarded `recordMessageAnalytics` call (see `finalize-steps.ts`). A turn that stops or errors mid-stream still records whichever marks it reached (e.g. `firstThinkingMs` set, `firstTokenMs` null) through the same guarded path, decoupled from the heavier turn-state projection (working set, task-state checkpoint) that a stopped turn otherwise skips.

The schema change is additive only (three new nullable columns; no rename, no drop). No new AI SDK UI stream part names are introduced — this amendment persists marks the server already computed and previously only logged via `console.info`, it does not add a new wire contract.
