...
**Implementation Status, 2026-06-25:** The refactor (commit 6936d067d) removed `mirrorMessage` to fix bloat but shipped with only Tier 1 regex-based Immediate Memory Admission, leaving Honcho sessions effectively empty. This accidentally built a parallel local store as the primary memory authority, violating ADR 0011's "no parallel local persona-memory system" rule. ADR 0039 (Deferred Memory Extraction restores Honcho as substrate with LLM-assisted Tier 2 intake) restores Honcho as the Memory Authority Substrate through projection-first, Honcho-reconcile-later architecture. The Memory Profile Projection remains the active read model for prompt-time personalization; Honcho receives only curated conclusions from admitted memories, written asynchronously during bounded maintenance reconciliation.
