...
The service owns shared upload limit resolution, optional conversation validation, uploaded source persistence through the Knowledge store, normalized-document extraction, Honcho sync with normalized-text fallback, prompt-readiness resolution, and upload trace output. Routes may authenticate, parse HTTP metadata, receive multipart form data, stream raw bytes into a temporary file, persist and assemble chunks, and translate service errors into HTTP responses. They should not import Knowledge store persistence helpers, Honcho sync helpers, or conversation lookup for durable upload completion.
