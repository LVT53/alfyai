...
> **Updated by [ADR-0052](0052-replace-searxng-web-research-with-parallel-search.md) (2026-07-13).** Where this ADR names the `web-research` service as owning source discovery/page opening, that service has been replaced by the Parallel-backed `parallel-search` service (search via `research_web`, page reads via `fetch_url`). The context-selection boundary decision here is unchanged; only the web backend it composes changed.
