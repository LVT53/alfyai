...
> **Partly superseded, 2026-09-06 (on-demand skill loading).** Skills no longer activate through durable **Skill Sessions**, and skills no longer write **Skill Notes**: `skill_sessions`, `skill_session_milestones`, `skill_note_operations`, and `skill_note_checkpoints` were dropped (`drizzle/1777140000088_drop_skill_sessions_and_notes.sql`). A turn now carries a compact `## Skills available` catalogue line, and the model loads one skill's full instructions for that turn through the `use_skill` tool; an explicit `$` selection forces the same instruction envelope into the same turn (`src/lib/server/services/skills/prompt-context.ts`). The Skill Session Panel, the session transition and note-intent envelope operations, and the "durable session survives refresh", "Skill Questions", and "note-capable skill" scenarios below no longer describe the system. Composer commands, `$` skill selection, skill ownership/permissioning, the Skill Draft flow, and the Skill Control Envelope's `skill_draft` operation are unchanged.
