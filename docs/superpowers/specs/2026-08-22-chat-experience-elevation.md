...
- **OpenRouteService (ORS), self-hosted from the start** — one service covers directions **+ matrix + geocoding + isochrones** on OpenStreetMap data, entirely on-box. Nothing leaves the server. **Ops step (orchestrator does this on the prod box during Tier D):** via `ssh alfyroot` (root — this lives in the **system `/opt`**), stand up an ORS Docker container under **`/opt/openrouteservice`** on alfyws (NOT under `/home/alfydesign`), built from an OSM extract (start with a regional extract, e.g. the relevant country, not planet — RAM/disk sane), and point the app at it via config (`ORS_BASE_URL` → the local container). No hosted-tier phase.
